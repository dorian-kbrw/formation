  ...
  render(){
    ...
    return <JSXZ in="orders" sel=".orders-container">
      <Z sel=".orders-column">
          ...
          <Z sel=".address">{order.custom.billing_address.street[0]}, {order.custom.billing_address.postcode} {order.custom.billing_address.city}</Z>
...
var Order = createReactClass({
  render(){
    ...
    return <JSXZ in="order" sel=".layout">
      ...
      <Z sel=".detail_address">{order.custom.billing_address.street[0]}, {order.custom.billing_address.postcode} {order.custom.billing_address.city}</Z>
      <Z sel=".detail_command">{order.remoteid}</Z>
      <Z sel=".goback" onClick={goHome}>Go Back</Z>
      <Z sel=".item-lines">
        {
          order.custom.items.map( item => (<JSXZ in="order" key={i++} sel=".item">
